-- Add ORDER BY to get_aggregated_tokens_data for stable pagination.
-- PostgREST/Supabase caps RPC results; we paginate with .range(). Order must be deterministic.
...
COMMENT ON FUNCTION get_aggregated_tokens_data IS 'Aggregates tokens data by time period and product for chart queries. Returns pre-aggregated data instead of raw rows for performance. ORDER BY ensures stable pagination.';
