...
interface HoursByProductChartProps {
  accountId: string
  accountSlug: string
...
export default function HoursByProductChart({
  accountId,
  accountSlug,
  granularity,
  productDisplayNames,
  ...
  // Fetch date range on mount (products will be derived from hours data)
  useEffect(() => {
    async function loadInitialData() {
      const supabase = createClient()
      const dateRange = await getDateRangeForHours(accountId, supabase)
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // If cache key hasn't changed (same granularity and date range), use cached data (including empty)
      if (!cacheKeyChanged) {
      ...
      try {
        ...
        const response = await fetch(`/api/accounts/${accountSlug}/dashboard/hours-chart-data?${params.toString()}`)
        
        if (!response.ok) {
          throw new Error(`Failed to fetch hours chart data: ${response.statusText}`)
        }

        const fullChartData: ChartDataPoint[] = await response.json()
        
        // Extract products from the hours data (these are the products that actually have hours data)
        const productsInData = new Set<string>()
        fullChartData.forEach(point => {
          Object.keys(point).forEach(key => {
            if (key !== 'date' && typeof point[key] === 'number') productsInData.add(key)
          })
        })
        const productsFromData = Array.from(productsInData).sort()
        
        // Update available products from the data (only if they changed or if we don't have any yet)
        if (availableProducts.length === 0 || JSON.stringify([...availableProducts].sort()) !== JSON.stringify(productsFromData)) {
          setAvailableProducts(productsFromData)
          // If no products are selected yet, or if selected products don't match available, select all
          if (selectedProducts.length === 0 || !selectedProducts.every(p => productsFromData.includes(p))) {
            setSelectedProducts(productsFromData)
          }
        }
        ...
        const productFilter = selectedProducts.length > 0 ? selectedProducts : productsFromData
        const filteredData = filterCachedData(
          fullChartData,
          productFilter,
          startDate,
        ...
        console.error('Error loading hours chart data:', error)
        setData([])
      } finally {
        setLoading(false)
      }
